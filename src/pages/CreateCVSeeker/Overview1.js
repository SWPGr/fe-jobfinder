...
const Overview1 = () => {
    ...
    useEffect(() => {
        const fetchJobs = async () => {
            try {
                const { jobs: apiJobs, pagination: apiPagination } = await EmployerService.fetchMyJobFake(pageNumber, pageSize);
                setJobs(apiJobs);
                setPagination(apiPagination);
                // Tính tổng số đơn ứng tuyển trên trang hiện tại
                const totalApps = apiJobs.reduce((sum, job) => sum + (job.jobApplicationCounts || 0), 0);
