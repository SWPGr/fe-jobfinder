...
const Application = ({ fullName, email, appliedDate, handleSelect, handleDownloadCV }) => (
...
const JobApplications = ({ jobId }) => {
                ...
                if (mounted) {
                    setApplications(
                        apps.map((app) => ({
                            ...
                            experience:
                                app.experience || app.jobSeeker?.experienceName || app.job?.experience?.name || '',
                            education: app.education || app.jobSeeker?.educationName || app.job?.education?.name || '',
                            salary:
                                app.salary ||
                            ...
                            appliedDate: app.appliedAt ? new Date(app.appliedAt).toLocaleString() : '',
                            originalData: app,
                        })),
                    );
                }
    ...
    const filteredApps = applications
        .filter(
            (app) =>
                (!filters.fullName || app.fullName?.toLowerCase().includes(filters.fullName.toLowerCase())) &&
                (!filters.title || app.title?.toLowerCase().includes(filters.title.toLowerCase())) &&
                (!filters.experience || (app.experience && app.experience.includes(filters.experience))) &&
                (!filters.salary || (app.salary && app.salary.toString().includes(filters.salary))) &&
                (!filters.jobType ||
                    (app.jobType && app.jobType.toLowerCase().includes(filters.jobType.toLowerCase()))) &&
                (!filters.education ||
                    (app.education && app.education.toLowerCase().includes(filters.education.toLowerCase()))) &&
                (!filters.jobLevel ||
                    (app.jobLevel && app.jobLevel.toLowerCase().includes(filters.jobLevel.toLowerCase()))),
        )
        .sort((a, b) =>
            sortOrder === 'newest'
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.tabs}>
                <button className={styles.active}>All Applications ({applications.length})</button>
            </div>
                ...
                {selectedApplicant && (
                    ...
                    >
                        ...
                        >
                            ...
                            {showResumeProfile && <ResumeProfile jobId={jobId} />}
                        </div>
                    </div>
                )}
            </div>
